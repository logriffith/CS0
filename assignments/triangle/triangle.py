...
## Before we continue in the process, we need to make sure that these inputed lengths are positive real numbers. If not, then the program will not run. Also, it doesn't make sense to compute the area and perimeter of a triangle if the given lengths don't even form a triangle. Thus, we will next test to see if these lengths form a triangle using the Triangle Inequality. The Triangle Inequality states that if a and b are real numbers, then |a+b|<=|a|+|b|. We will use this fact to verify the lengths. If one combibation of sides a and b violates the Triangle Inequality, then these side lengths do not form a triangle. Note that the is_triangle function defined above uses the Triangle Inequality to verify that the given sides form a triangle.
